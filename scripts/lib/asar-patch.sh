...
patch_asar() {
    ...
    cd "$WORK_DIR"
    run_asar extract "$resources_dir/app.asar" app-extracted
    ...
    run_asar pack app-extracted app.asar --ordering "$WORK_DIR/app.asar.ordering" --unpack "{*.node,*.so,*.dylib}" 2>/dev/null
...
inspect_rebuild_candidate() {
    ...
    cd "$WORK_DIR"
    run_asar extract "$resources_dir/app.asar" "$inspect_dir"
