...
function applyLinuxBundledPluginCopyPermissionsPatch(currentSource) {
  ...
  const helpers = [
    `async function ${ancestorHelperName}(e,t){let n=await t.realpath(e),r=process.geteuid?.(),i=null;if(!Number.isInteger(r))throw Error(\`Linux bundled plugin path is not trusted\`);if(typeof process.env?.CODEX_LINUX_APP_ID===\`string\`&&process.env.CODEX_LINUX_APP_ID!==\`codex-desktop\`&&typeof process.resourcesPath===\`string\`){let e=await t.realpath(process.resourcesPath);(n===e||n.startsWith(e+${pathVar}.sep))&&(i=e)}for(let e=n;;){let n=await t.lstat(e),a=n.mode;if(n.isSymbolicLink()||!n.isDirectory()||n.uid!==r&&n.uid!==0||a&18&&!(n.uid===0&&a&512))throw Error(\`Linux bundled plugin path is not trusted\`);if(e===i)break;let o=(0,${pathVar}.dirname)(e);if(o===e)break;e=o}return n}`,
