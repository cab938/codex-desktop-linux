...
function applyLinuxBundledPluginCopyPermissionsPatch(currentSource) {
  ...
  const helpers = [
    `async function ${ancestorHelperName}(e,t){let n=await t.realpath(e),r=process.geteuid?.(),i=null;if(!Number.isInteger(r))throw Error(\`Linux bundled plugin path is not trusted\`);if(typeof process.env?.CODEX_LINUX_APP_ID===\`string\`&&process.env.CODEX_LINUX_APP_ID!==\`codex-desktop\`)for(let e of[process.resourcesPath,process.env.CODEX_HOME])if(typeof e===\`string\`)try{let r=await t.realpath(e);if(n===r||n.startsWith(r+${pathVar}.sep)){i=r;break}}catch{}for(let e=n;;){let n=await t.lstat(e),a=n.mode;if(n.isSymbolicLink()||!n.isDirectory()||n.uid!==r&&n.uid!==0||a&18&&!(n.uid===0&&a&512))throw Error(\`Linux bundled plugin path is not trusted\`);if(e===i)break;let o=(0,${pathVar}.dirname)(e);if(o===e)break;e=o}return n}`,
