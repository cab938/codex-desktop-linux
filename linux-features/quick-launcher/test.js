...
test("asset patterns select only the current intended chunk families", () => {
  assert.equal(
    CONTEXT_ASSET_PATTERN.test(
      "app-initial~artifact-tab-content.electron~notebook-preview-panel~app-main~business-checkout~oxnpxkxc-hash.js",
    ),
    true,
  );
  assert.equal(
    CONTEXT_ASSET_PATTERN.test(
      "app-initial~artifact-tab-content.electron~notebook-preview-panel~app-main~business-checkout~other-hash.js",
    ),
    false,
  );
